...
const processLines = (lines: Line[], gridWidth: number, gridHeight: number) => {
  const grid: Grid = new Array(gridHeight)
    .fill(null)
...
const countOverlaps = (grid: Grid) => {
  return grid
    .map((row) => row.filter((numLines) => numLines >= 2).length)
    .reduce((a, b) => a + b);
};

const lines = readLines("../inputs/day05.txt");
const grid = processLines(lines, 1000, 1000);
